...
static void PrintTotalRecord(struct LinkBattleRecords * records)
{
    ...
    if (gSaveBlock2Ptr->optionsLanguage == SPA)
		StringExpandPlaceholders(gStringVar4, gText_BattleRecords_TotalRecordSpa);
...
static void PrintOpponentBattleRecord(struct LinkBattleRecord * record, u8 y)
{
    ...
    if (record->wins == 0 && record->losses == 0 && record->draws == 0)
    {
        ...
        if (gSaveBlock2Ptr->optionsLanguage == SPA)
			AddTextPrinterParameterized4(0, 2, 0, y, 0, 2, sTextColor, 0, gText_BattleRecords_7DashesSpa);
        for (i = 0; i < 3; i++)
        {
            if (i == 0)
                x = 0x54;
            ...
            if (gSaveBlock2Ptr->optionsLanguage == SPA)
				AddTextPrinterParameterized4(0, 2, x, y, 0, 2, sTextColor, 0, gText_BattleRecords_4DashesSpa);
        }
    }
    else
    {
...
static void PrintBattleRecords(void)
{
    ...
    if (gSaveBlock2Ptr->optionsLanguage == SPA)
		StringExpandPlaceholders(gStringVar4, gText_BattleRecords_PlayersBattleResultsSpa);
    ...
    if (gSaveBlock2Ptr->optionsLanguage == SPA)
		AddTextPrinterParameterized4(0, 2, 0x54, 0x30, 0, 2, sTextColor, 0, gText_BattleRecords_ColumnHeadersSpa);
    for (i = 0; i < LINK_B_RECORDS_COUNT; i++)
